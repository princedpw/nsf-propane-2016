...
example).  Our research will directly exploit and build upon this research, especially
the work of Goldenberg \emph{et al}~\cite{Goldenberg:multi-home}.
What is new is the design of
sophisticated, declarative, high-level \emph{programming languages} that 
(1) specify routes (including inter-domain routes), (2) back-ups for fault tolerance, and 
...
policing and steering of \emph{intra}-domain traffic, capacities, congestion and
management of middleboxes.  They have not tackled 
...
practical policies can we cover?  How do we modularize policy that combines 
routing, preferences, fault tolerance, cost and contracts?
...
\item How do we engineer system interfaces to ensure a clean separation between optimization
infrastructure and the rest of the compiler?
\item How do we compute deltas between one configuration and the next?  How do we install deltas
while ensuring minimal disruption?  What form of \emph{consistent update}~\cite{reitblatt+:consistent-updates} should be used here?
\item Can we make use of heterogeneous back-ends (as discussed in the next subsection) to implement 
a combination of load balancing and inter-domain routing effectively and automaticallly? 
\end{itemize}
...
\noindent
{\bf George Varghese:} George Varghese has worked in industry over the last 5 years and has not had NSF support during this time.  Before that, he was awarded 10 NSF grants, which have led to networking innovations such as Deficit Round Robin, IP lookups, and automated worm fingerprinting.
