...
However, it costs significant human time and effort to translate low-level configurations into a higher-level policy and to ensure that the original functionality and performance is preserved.  Moreover, migrating all operations at once is typically
both logistically impossible.   In our discussions with network engineers it has become clear that these issues are key barriers to practical deployment of new approaches to network configuration technology.  To address these problems, we plan to develop automated tools to help network engineers to \emph{incrementally} migrate existing subnetworks to the \Name platform.  In addition, we will
...
migration to proceed in a slow, controlled fashion, but will also allow nervous engineers to test \Name on a tiny fraction of their
...
intermediate format, and then a decision procedure that tests for equivalence, and possibly other properties, will be applied.
...
sent from external peers, (2) a concrete packet, and (3) the path each policy causes the packet to use in the given environment.
The engineer would use this information to iteratively revise the 
...
wide range of features and (2) limited in the properties that they can analyze.
...
undesirable, but stable solution.  Restoration of the failure does not cause the network to shift back to the prior preferred
routing solution.}  
...
$z$ cannot be used or $x$ prefers $y$ to $z$.  We believe this encoding will be efficient because it avoids having to reason
explicitly about message ordering.  Instead, it characterizes the possible solutions to the routing problem --- the stable paths.
...
to a network operator.  We  will also measure the scaling limits of our approach empiricially and test its generality.
...
we will also consider approaches to partially or completely {\em infer} a \Name policy directly from the given
low-level configuration.  One approach is to use the logical encoding of
the low-level configuration, and an SMT solver, to help search for 
collections of higher-level constraints that have the same semantics as the low-level configuration.
Such SMT-based search procedures, such as those integrated into the Sketch~\cite{sketch} 
and Rosette~\cite{rosette} tools have been highly effective in other domains.
A second approach, since \Name policies employ regular
...
We will likely need to extend existing
...
The \Name platform then can {\em assume} that these specifications holds when reasoning about the network as a whole and compiling the \Name-specific portions.  Separately, we must {\em guarantee} that the manually configured portions indeed meet their interface specifications.
...
Alternatively, an interface might guarantee isolation properties of subnetworks. In general, interfaces 
...
Batfish~\cite{batfish}, anther control-plane analysis tool, is a \emph{testing} tool as opposed to a \emph{verification}. 
Batfish may be used to find bugs in configurations but does not guarantee equivalence of control planes.

There is a great deal of work on \emph{data plane} verification, such as work on AntEater~\cite{mai+:anteater}, Header Space Analysis~\cite{hsa},
...
verification efforts that we are interested in, which focus on \emph{control plane} protocol configuration.
...
\item Can SMT-based synthesis or RE-learning techniques help us infer \Name policies directly from low-level configurations?
