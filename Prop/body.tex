...
However, it costs significant human time and effort to translate low-level configurations into a higher-level policy and to ensure that the original functionality and performance is preserved.  Moreover, migrating all ones operations at once is typically
both logistically impossible and risky.   In our discussions with network engineers it has become clear that these issues are key barriers to practical deployment of new approaches to network configuration technology.  To address these problems, we plan to develop automated tools to help network engineers to \emph{incrementally} migrate existing subnetworks to the \Name platform.  In addition, we will
support mixed-mode verification and operations technology so that networks can safely
run a combination of legacy-controlled and \Name-controlled networks.  
...
\paragraph{Migration}  Figure~\ref{fig:transition-tech} shows a proposed architecture for tool-supported migration to the \Name platform.  The high-level idea is that a network engineer would 
provide a \Name policy for the network and that policy would be compared to
the current low-level network configuration.  In order to perform the comparison, the
high-level and low-level configurations would have to be converted into a common 
intermediate format, and then a decision procedure would be applied.
When the two policies differ, the system would automatically generate
concrete \emph{counter-examples}.  These counter-examples would 
include: (1) a network policy environment, including a failure scenario and advertisements
sent from external peers, (2) a concrete packet, and (3) the path in each policy that this
packet travels along in the given environment.
The engineer would use this information to iteratively revise the candidate \Name policy until it achieves the desired correspondence with the legacy configuration.  
...
Stability & Unique & No & No & Yes \\
 & Wedgie & No & No & Yes \\\hline
  \end{tabular}
\caption{Comparison with of our proposed work (Prop) with
two recent control plane verification technologies: ERA (OSDI 2016)
and ARC (SIGCOMM 2016)}
\label{fig:smt-vs-arc}
\end{wrapfigure}

The first major decision in realizing this architecture is to choose an effective
intermediate language for representing both low-level legacy configurations and
high-level policy.  In other words:  What is the common format?  
This intermediate language must be capable of expressing the semantics
of both kinds of policies and must be amenable to equivalence testing, and perhaps
other kinds of less exacting tests (such as path inclusion testing --- the paths
of an implementation should be a subset of the paths of a more abstract specification).
One possible choice is to use the config language of the legacy network as the ``intermediate'' 
format.  After all, one can just compile the \Name policy into a concrete configuration.
Then one might try to use existing verification tools such as ERA~\cite{era} or ARC~\cite{arc} to test for
equivalence.  However, this approach has several disadvantages.  First,
existing control-plane verification tools are incapable of representing a sufficiently
wide range of features.  As far as we are aware, no verification tool can handle
core BGP features such as regular expression filters and communities, general interaction with
other protocols via route redistribution or iBGP.  A second disadvantage is that existing tools 
verify a subset of the properties needed for migration.  
Some tools do not perform basic \emph{equivalence} testing.  Other tools do
not ensure \emph{preferences} and backup paths are respected, do not check for important convergence
properties such as the existence of a \emph{unique} stable solution or the non-existence of
a \emph{wedgie}~\cite{wedgie} --- all facts operators need to know to ensure safe migration.
Figure~\ref{fig:smt-vs-arc} summarizes
a few key differences between the two recent verification efforts (ERA and ARC) and
our goals (Prop).  

\dpw{I'm kind of in the middle of this.}
