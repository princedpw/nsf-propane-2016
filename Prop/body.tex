...
RPSL~\cite{rpsl}, Nettle~\cite{nettle}, ConfigAssure~\cite{narain:lisa05,narain+:configassure}, and DADC~\cite{DADC} are three
...
our sub-language will allow description of the common 95/5 contract~\cite{routingplaybook}.  In this contract, the customer traffic is measured at 5-minute intervals and is billed at 
...
%% There are (at least) two ways for network operators to use such contract and costs specifications productively.  One way is as a \emph{model}
...
When such contract specifications are an integrated part of the network operator's 
policy specification language, they may be used to guide configuration synthesis.  
In this case,
...
involves building a controller to monitor traffic levels and push BGP
...
such as a switches running OpenFlow~\cite{openflow}, P4~\cite{P4} or PIFO~\cite{pifo}.
The following section (backend diversity), we explain our complimentary efforts 
...
Consequently, most networks use
